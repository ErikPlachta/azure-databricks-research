-- ============================================================================
-- 04_gold/04_materialized_views.sql
-- 50 gold mv<entity> across 5 PD teams.
--
-- Cascading-MV bodies (DECISIONS.md #13 — supersedes #6 for 0.1.1+):
--   * gold mv* references investments.mv* (silver MVs), NOT investments.v*.
--   * Intra-gold IN-clause refs use mv* in mv bodies, v* in v bodies.
--   * Mechanically derivable from gold v* via s/v/mv/g substitution.
-- ============================================================================

DECLARE OR REPLACE VARIABLE catalog_name STRING DEFAULT 'medallion_demo';
EXECUTE IMMEDIATE 'USE CATALOG ' || catalog_name;

-- team_pd_direct_lending ------------------------------------------------------
CREATE OR REPLACE MATERIALIZED VIEW team_pd_direct_lending.mvposition_analytics_fact AS
WITH team_positions AS (
    SELECT p.* FROM investments.mvposition_analytics_fact p
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_pd_direct_lending'
)
SELECT position_sk, enterprise_key, position_date, portfolio_sk, security_sk, business_unit_sk,
    quantity, market_value_usd, book_value_usd, cost_basis_usd, unrealized_gl_usd,
    market_value_usd / NULLIF(SUM(market_value_usd) OVER (PARTITION BY position_date), 0) AS concentration_pct,
    CAST(DENSE_RANK() OVER (PARTITION BY position_date ORDER BY market_value_usd DESC) AS INT) AS portfolio_rank,
    currency_code, silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM team_positions;

CREATE OR REPLACE MATERIALIZED VIEW team_pd_direct_lending.mvcontract_details_fact AS
SELECT c.contract_details_sk, c.enterprise_key, c.detail_date, c.contract_sk, c.entity_sk,
    c.contract_type, c.principal_usd, c.coupon_rate, c.spread_over_benchmark,
    c.days_to_maturity, c.status, c.has_active_breach,
    YEAR(cd.signing_date) AS vintage_year,
    c.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvcontract_details_fact c
LEFT JOIN investments.mvcontract_dim cd ON cd.contract_sk = c.contract_sk AND cd.is_current = TRUE
WHERE c.contract_sk IN (
    SELECT DISTINCT cp.contract_sk FROM investments.mvtransactions_collateral_positions_fact cp
    JOIN investments.mvposition_analytics_fact p ON p.security_sk = cp.security_sk
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_pd_direct_lending'
);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_direct_lending.mvcontract_summary_fact AS
SELECT cs.contract_summary_sk, cs.enterprise_key, cs.summary_date, cs.contract_sk,
    cs.outstanding_principal_usd, cs.accrued_interest_usd, cs.paid_to_date_usd,
    cs.performance_status, cs.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvcontract_summary_fact cs
WHERE cs.contract_sk IN (SELECT DISTINCT contract_sk FROM team_pd_direct_lending.mvcontract_details_fact);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_direct_lending.mvportfolio_analytics_fact AS
SELECT pa.portfolio_analytics_sk, pa.analytics_date, pa.portfolio_sk, pa.business_unit_sk,
    pa.var_95, pa.var_99, pa.expected_shortfall, pa.period_return_pct, pa.ytd_return_pct,
    pa.benchmark_return_pct, pa.benchmark_code, pa.silver_loaded_at,
    current_timestamp() AS gold_loaded_at
FROM investments.mvportfolio_analytics_fact pa
JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = pa.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_pd_direct_lending';

CREATE OR REPLACE MATERIALIZED VIEW team_pd_direct_lending.mvsecurity_dim AS
WITH team_secs AS (
    SELECT DISTINCT p.security_sk FROM investments.mvposition_analytics_fact p
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_pd_direct_lending'
)
SELECT s.security_sk, s.enterprise_key, s.effective_start_date, s.effective_end_date, s.is_current,
    s.security_name, s.asset_class, s.sub_asset_class, s.issue_date, s.maturity_date,
    s.coupon_rate, s.currency_code, s.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_dim s JOIN team_secs ts ON ts.security_sk = s.security_sk;

CREATE OR REPLACE MATERIALIZED VIEW team_pd_direct_lending.mvsecurity_master_fact AS
SELECT m.security_master_sk, m.snapshot_date, m.security_sk, m.enterprise_key, m.security_name,
    m.asset_class, m.latest_close_price_usd, m.days_to_maturity, m.is_matured,
    m.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_master_fact m
WHERE m.security_sk IN (SELECT security_sk FROM team_pd_direct_lending.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_direct_lending.mvsecurity_price_fact AS
SELECT sp.security_price_sk, sp.enterprise_key, sp.price_date, sp.security_sk,
    sp.close_price_usd, sp.currency_code, sp.price_type,
    sp.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_price_fact sp
WHERE sp.security_sk IN (SELECT security_sk FROM team_pd_direct_lending.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_direct_lending.mvsecurity_rating_dim AS
SELECT sr.security_rating_sk, sr.enterprise_key, sr.effective_start_date, sr.effective_end_date,
    sr.is_current, sr.security_enterprise_key, sr.rating_agency, sr.rating_value,
    sr.rating_outlook, sr.rating_numeric_score,
    sr.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_rating_dim sr
WHERE sr.security_enterprise_key IN (SELECT enterprise_key FROM team_pd_direct_lending.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_direct_lending.mvtransactions_collateral_exposure_fact AS
SELECT ce.collateral_exposure_sk, ce.enterprise_key, ce.exposure_date, ce.contract_sk,
    ce.exposure_amount_usd, ce.collateral_value_usd, ce.collateral_type, ce.ltv_pct,
    ce.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvtransactions_collateral_exposure_fact ce
WHERE ce.contract_sk IN (SELECT DISTINCT contract_sk FROM team_pd_direct_lending.mvcontract_details_fact);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_direct_lending.mvtransactions_collateral_positions_fact AS
SELECT cp.collateral_position_sk, cp.enterprise_key, cp.position_date, cp.contract_sk,
    cp.security_sk, cp.position_value_usd, cp.collateral_role,
    cp.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvtransactions_collateral_positions_fact cp
WHERE cp.contract_sk IN (SELECT DISTINCT contract_sk FROM team_pd_direct_lending.mvcontract_details_fact);

-- team_pd_distressed ----------------------------------------------------------
CREATE OR REPLACE MATERIALIZED VIEW team_pd_distressed.mvposition_analytics_fact AS
WITH team_positions AS (
    SELECT p.* FROM investments.mvposition_analytics_fact p
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_pd_distressed'
)
SELECT position_sk, enterprise_key, position_date, portfolio_sk, security_sk, business_unit_sk,
    quantity, market_value_usd, book_value_usd, cost_basis_usd, unrealized_gl_usd,
    market_value_usd / NULLIF(SUM(market_value_usd) OVER (PARTITION BY position_date), 0) AS concentration_pct,
    CAST(DENSE_RANK() OVER (PARTITION BY position_date ORDER BY market_value_usd DESC) AS INT) AS portfolio_rank,
    currency_code, silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM team_positions;

CREATE OR REPLACE MATERIALIZED VIEW team_pd_distressed.mvcontract_details_fact AS
SELECT c.contract_details_sk, c.enterprise_key, c.detail_date, c.contract_sk, c.entity_sk,
    c.contract_type, c.principal_usd, c.coupon_rate, c.spread_over_benchmark,
    c.days_to_maturity, c.status, c.has_active_breach,
    YEAR(cd.signing_date) AS vintage_year,
    c.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvcontract_details_fact c
LEFT JOIN investments.mvcontract_dim cd ON cd.contract_sk = c.contract_sk AND cd.is_current = TRUE
WHERE c.contract_sk IN (
    SELECT DISTINCT cp.contract_sk FROM investments.mvtransactions_collateral_positions_fact cp
    JOIN investments.mvposition_analytics_fact p ON p.security_sk = cp.security_sk
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_pd_distressed'
);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_distressed.mvcontract_summary_fact AS
SELECT cs.contract_summary_sk, cs.enterprise_key, cs.summary_date, cs.contract_sk,
    cs.outstanding_principal_usd, cs.accrued_interest_usd, cs.paid_to_date_usd,
    cs.performance_status, cs.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvcontract_summary_fact cs
WHERE cs.contract_sk IN (SELECT DISTINCT contract_sk FROM team_pd_distressed.mvcontract_details_fact);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_distressed.mvportfolio_analytics_fact AS
SELECT pa.portfolio_analytics_sk, pa.analytics_date, pa.portfolio_sk, pa.business_unit_sk,
    pa.var_95, pa.var_99, pa.expected_shortfall, pa.period_return_pct, pa.ytd_return_pct,
    pa.benchmark_return_pct, pa.benchmark_code, pa.silver_loaded_at,
    current_timestamp() AS gold_loaded_at
FROM investments.mvportfolio_analytics_fact pa
JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = pa.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_pd_distressed';

CREATE OR REPLACE MATERIALIZED VIEW team_pd_distressed.mvsecurity_dim AS
WITH team_secs AS (
    SELECT DISTINCT p.security_sk FROM investments.mvposition_analytics_fact p
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_pd_distressed'
)
SELECT s.security_sk, s.enterprise_key, s.effective_start_date, s.effective_end_date, s.is_current,
    s.security_name, s.asset_class, s.sub_asset_class, s.issue_date, s.maturity_date,
    s.coupon_rate, s.currency_code, s.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_dim s JOIN team_secs ts ON ts.security_sk = s.security_sk;

CREATE OR REPLACE MATERIALIZED VIEW team_pd_distressed.mvsecurity_master_fact AS
SELECT m.security_master_sk, m.snapshot_date, m.security_sk, m.enterprise_key, m.security_name,
    m.asset_class, m.latest_close_price_usd, m.days_to_maturity, m.is_matured,
    m.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_master_fact m
WHERE m.security_sk IN (SELECT security_sk FROM team_pd_distressed.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_distressed.mvsecurity_price_fact AS
SELECT sp.security_price_sk, sp.enterprise_key, sp.price_date, sp.security_sk,
    sp.close_price_usd, sp.currency_code, sp.price_type,
    sp.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_price_fact sp
WHERE sp.security_sk IN (SELECT security_sk FROM team_pd_distressed.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_distressed.mvsecurity_rating_dim AS
SELECT sr.security_rating_sk, sr.enterprise_key, sr.effective_start_date, sr.effective_end_date,
    sr.is_current, sr.security_enterprise_key, sr.rating_agency, sr.rating_value,
    sr.rating_outlook, sr.rating_numeric_score,
    sr.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_rating_dim sr
WHERE sr.security_enterprise_key IN (SELECT enterprise_key FROM team_pd_distressed.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_distressed.mvtransactions_collateral_exposure_fact AS
SELECT ce.collateral_exposure_sk, ce.enterprise_key, ce.exposure_date, ce.contract_sk,
    ce.exposure_amount_usd, ce.collateral_value_usd, ce.collateral_type, ce.ltv_pct,
    ce.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvtransactions_collateral_exposure_fact ce
WHERE ce.contract_sk IN (SELECT DISTINCT contract_sk FROM team_pd_distressed.mvcontract_details_fact);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_distressed.mvtransactions_collateral_positions_fact AS
SELECT cp.collateral_position_sk, cp.enterprise_key, cp.position_date, cp.contract_sk,
    cp.security_sk, cp.position_value_usd, cp.collateral_role,
    cp.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvtransactions_collateral_positions_fact cp
WHERE cp.contract_sk IN (SELECT DISTINCT contract_sk FROM team_pd_distressed.mvcontract_details_fact);

-- team_pd_mezzanine -----------------------------------------------------------
CREATE OR REPLACE MATERIALIZED VIEW team_pd_mezzanine.mvposition_analytics_fact AS
WITH team_positions AS (
    SELECT p.* FROM investments.mvposition_analytics_fact p
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_pd_mezzanine'
)
SELECT position_sk, enterprise_key, position_date, portfolio_sk, security_sk, business_unit_sk,
    quantity, market_value_usd, book_value_usd, cost_basis_usd, unrealized_gl_usd,
    market_value_usd / NULLIF(SUM(market_value_usd) OVER (PARTITION BY position_date), 0) AS concentration_pct,
    CAST(DENSE_RANK() OVER (PARTITION BY position_date ORDER BY market_value_usd DESC) AS INT) AS portfolio_rank,
    currency_code, silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM team_positions;

CREATE OR REPLACE MATERIALIZED VIEW team_pd_mezzanine.mvcontract_details_fact AS
SELECT c.contract_details_sk, c.enterprise_key, c.detail_date, c.contract_sk, c.entity_sk,
    c.contract_type, c.principal_usd, c.coupon_rate, c.spread_over_benchmark,
    c.days_to_maturity, c.status, c.has_active_breach,
    YEAR(cd.signing_date) AS vintage_year,
    c.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvcontract_details_fact c
LEFT JOIN investments.mvcontract_dim cd ON cd.contract_sk = c.contract_sk AND cd.is_current = TRUE
WHERE c.contract_sk IN (
    SELECT DISTINCT cp.contract_sk FROM investments.mvtransactions_collateral_positions_fact cp
    JOIN investments.mvposition_analytics_fact p ON p.security_sk = cp.security_sk
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_pd_mezzanine'
);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_mezzanine.mvcontract_summary_fact AS
SELECT cs.contract_summary_sk, cs.enterprise_key, cs.summary_date, cs.contract_sk,
    cs.outstanding_principal_usd, cs.accrued_interest_usd, cs.paid_to_date_usd,
    cs.performance_status, cs.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvcontract_summary_fact cs
WHERE cs.contract_sk IN (SELECT DISTINCT contract_sk FROM team_pd_mezzanine.mvcontract_details_fact);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_mezzanine.mvportfolio_analytics_fact AS
SELECT pa.portfolio_analytics_sk, pa.analytics_date, pa.portfolio_sk, pa.business_unit_sk,
    pa.var_95, pa.var_99, pa.expected_shortfall, pa.period_return_pct, pa.ytd_return_pct,
    pa.benchmark_return_pct, pa.benchmark_code, pa.silver_loaded_at,
    current_timestamp() AS gold_loaded_at
FROM investments.mvportfolio_analytics_fact pa
JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = pa.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_pd_mezzanine';

CREATE OR REPLACE MATERIALIZED VIEW team_pd_mezzanine.mvsecurity_dim AS
WITH team_secs AS (
    SELECT DISTINCT p.security_sk FROM investments.mvposition_analytics_fact p
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_pd_mezzanine'
)
SELECT s.security_sk, s.enterprise_key, s.effective_start_date, s.effective_end_date, s.is_current,
    s.security_name, s.asset_class, s.sub_asset_class, s.issue_date, s.maturity_date,
    s.coupon_rate, s.currency_code, s.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_dim s JOIN team_secs ts ON ts.security_sk = s.security_sk;

CREATE OR REPLACE MATERIALIZED VIEW team_pd_mezzanine.mvsecurity_master_fact AS
SELECT m.security_master_sk, m.snapshot_date, m.security_sk, m.enterprise_key, m.security_name,
    m.asset_class, m.latest_close_price_usd, m.days_to_maturity, m.is_matured,
    m.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_master_fact m
WHERE m.security_sk IN (SELECT security_sk FROM team_pd_mezzanine.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_mezzanine.mvsecurity_price_fact AS
SELECT sp.security_price_sk, sp.enterprise_key, sp.price_date, sp.security_sk,
    sp.close_price_usd, sp.currency_code, sp.price_type,
    sp.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_price_fact sp
WHERE sp.security_sk IN (SELECT security_sk FROM team_pd_mezzanine.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_mezzanine.mvsecurity_rating_dim AS
SELECT sr.security_rating_sk, sr.enterprise_key, sr.effective_start_date, sr.effective_end_date,
    sr.is_current, sr.security_enterprise_key, sr.rating_agency, sr.rating_value,
    sr.rating_outlook, sr.rating_numeric_score,
    sr.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_rating_dim sr
WHERE sr.security_enterprise_key IN (SELECT enterprise_key FROM team_pd_mezzanine.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_mezzanine.mvtransactions_collateral_exposure_fact AS
SELECT ce.collateral_exposure_sk, ce.enterprise_key, ce.exposure_date, ce.contract_sk,
    ce.exposure_amount_usd, ce.collateral_value_usd, ce.collateral_type, ce.ltv_pct,
    ce.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvtransactions_collateral_exposure_fact ce
WHERE ce.contract_sk IN (SELECT DISTINCT contract_sk FROM team_pd_mezzanine.mvcontract_details_fact);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_mezzanine.mvtransactions_collateral_positions_fact AS
SELECT cp.collateral_position_sk, cp.enterprise_key, cp.position_date, cp.contract_sk,
    cp.security_sk, cp.position_value_usd, cp.collateral_role,
    cp.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvtransactions_collateral_positions_fact cp
WHERE cp.contract_sk IN (SELECT DISTINCT contract_sk FROM team_pd_mezzanine.mvcontract_details_fact);

-- team_pd_real_estate_debt ----------------------------------------------------
CREATE OR REPLACE MATERIALIZED VIEW team_pd_real_estate_debt.mvposition_analytics_fact AS
WITH team_positions AS (
    SELECT p.* FROM investments.mvposition_analytics_fact p
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_pd_real_estate_debt'
)
SELECT position_sk, enterprise_key, position_date, portfolio_sk, security_sk, business_unit_sk,
    quantity, market_value_usd, book_value_usd, cost_basis_usd, unrealized_gl_usd,
    market_value_usd / NULLIF(SUM(market_value_usd) OVER (PARTITION BY position_date), 0) AS concentration_pct,
    CAST(DENSE_RANK() OVER (PARTITION BY position_date ORDER BY market_value_usd DESC) AS INT) AS portfolio_rank,
    currency_code, silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM team_positions;

CREATE OR REPLACE MATERIALIZED VIEW team_pd_real_estate_debt.mvcontract_details_fact AS
SELECT c.contract_details_sk, c.enterprise_key, c.detail_date, c.contract_sk, c.entity_sk,
    c.contract_type, c.principal_usd, c.coupon_rate, c.spread_over_benchmark,
    c.days_to_maturity, c.status, c.has_active_breach,
    YEAR(cd.signing_date) AS vintage_year,
    c.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvcontract_details_fact c
LEFT JOIN investments.mvcontract_dim cd ON cd.contract_sk = c.contract_sk AND cd.is_current = TRUE
WHERE c.contract_sk IN (
    SELECT DISTINCT cp.contract_sk FROM investments.mvtransactions_collateral_positions_fact cp
    JOIN investments.mvposition_analytics_fact p ON p.security_sk = cp.security_sk
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_pd_real_estate_debt'
);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_real_estate_debt.mvcontract_summary_fact AS
SELECT cs.contract_summary_sk, cs.enterprise_key, cs.summary_date, cs.contract_sk,
    cs.outstanding_principal_usd, cs.accrued_interest_usd, cs.paid_to_date_usd,
    cs.performance_status, cs.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvcontract_summary_fact cs
WHERE cs.contract_sk IN (SELECT DISTINCT contract_sk FROM team_pd_real_estate_debt.mvcontract_details_fact);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_real_estate_debt.mvportfolio_analytics_fact AS
SELECT pa.portfolio_analytics_sk, pa.analytics_date, pa.portfolio_sk, pa.business_unit_sk,
    pa.var_95, pa.var_99, pa.expected_shortfall, pa.period_return_pct, pa.ytd_return_pct,
    pa.benchmark_return_pct, pa.benchmark_code, pa.silver_loaded_at,
    current_timestamp() AS gold_loaded_at
FROM investments.mvportfolio_analytics_fact pa
JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = pa.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_pd_real_estate_debt';

CREATE OR REPLACE MATERIALIZED VIEW team_pd_real_estate_debt.mvsecurity_dim AS
WITH team_secs AS (
    SELECT DISTINCT p.security_sk FROM investments.mvposition_analytics_fact p
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_pd_real_estate_debt'
)
SELECT s.security_sk, s.enterprise_key, s.effective_start_date, s.effective_end_date, s.is_current,
    s.security_name, s.asset_class, s.sub_asset_class, s.issue_date, s.maturity_date,
    s.coupon_rate, s.currency_code, s.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_dim s JOIN team_secs ts ON ts.security_sk = s.security_sk;

CREATE OR REPLACE MATERIALIZED VIEW team_pd_real_estate_debt.mvsecurity_master_fact AS
SELECT m.security_master_sk, m.snapshot_date, m.security_sk, m.enterprise_key, m.security_name,
    m.asset_class, m.latest_close_price_usd, m.days_to_maturity, m.is_matured,
    m.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_master_fact m
WHERE m.security_sk IN (SELECT security_sk FROM team_pd_real_estate_debt.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_real_estate_debt.mvsecurity_price_fact AS
SELECT sp.security_price_sk, sp.enterprise_key, sp.price_date, sp.security_sk,
    sp.close_price_usd, sp.currency_code, sp.price_type,
    sp.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_price_fact sp
WHERE sp.security_sk IN (SELECT security_sk FROM team_pd_real_estate_debt.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_real_estate_debt.mvsecurity_rating_dim AS
SELECT sr.security_rating_sk, sr.enterprise_key, sr.effective_start_date, sr.effective_end_date,
    sr.is_current, sr.security_enterprise_key, sr.rating_agency, sr.rating_value,
    sr.rating_outlook, sr.rating_numeric_score,
    sr.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_rating_dim sr
WHERE sr.security_enterprise_key IN (SELECT enterprise_key FROM team_pd_real_estate_debt.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_real_estate_debt.mvtransactions_collateral_exposure_fact AS
SELECT ce.collateral_exposure_sk, ce.enterprise_key, ce.exposure_date, ce.contract_sk,
    ce.exposure_amount_usd, ce.collateral_value_usd, ce.collateral_type, ce.ltv_pct,
    ce.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvtransactions_collateral_exposure_fact ce
WHERE ce.contract_sk IN (SELECT DISTINCT contract_sk FROM team_pd_real_estate_debt.mvcontract_details_fact);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_real_estate_debt.mvtransactions_collateral_positions_fact AS
SELECT cp.collateral_position_sk, cp.enterprise_key, cp.position_date, cp.contract_sk,
    cp.security_sk, cp.position_value_usd, cp.collateral_role,
    cp.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvtransactions_collateral_positions_fact cp
WHERE cp.contract_sk IN (SELECT DISTINCT contract_sk FROM team_pd_real_estate_debt.mvcontract_details_fact);

-- team_pd_specialty_finance ---------------------------------------------------
CREATE OR REPLACE MATERIALIZED VIEW team_pd_specialty_finance.mvposition_analytics_fact AS
WITH team_positions AS (
    SELECT p.* FROM investments.mvposition_analytics_fact p
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_pd_specialty_finance'
)
SELECT position_sk, enterprise_key, position_date, portfolio_sk, security_sk, business_unit_sk,
    quantity, market_value_usd, book_value_usd, cost_basis_usd, unrealized_gl_usd,
    market_value_usd / NULLIF(SUM(market_value_usd) OVER (PARTITION BY position_date), 0) AS concentration_pct,
    CAST(DENSE_RANK() OVER (PARTITION BY position_date ORDER BY market_value_usd DESC) AS INT) AS portfolio_rank,
    currency_code, silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM team_positions;

CREATE OR REPLACE MATERIALIZED VIEW team_pd_specialty_finance.mvcontract_details_fact AS
SELECT c.contract_details_sk, c.enterprise_key, c.detail_date, c.contract_sk, c.entity_sk,
    c.contract_type, c.principal_usd, c.coupon_rate, c.spread_over_benchmark,
    c.days_to_maturity, c.status, c.has_active_breach,
    YEAR(cd.signing_date) AS vintage_year,
    c.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvcontract_details_fact c
LEFT JOIN investments.mvcontract_dim cd ON cd.contract_sk = c.contract_sk AND cd.is_current = TRUE
WHERE c.contract_sk IN (
    SELECT DISTINCT cp.contract_sk FROM investments.mvtransactions_collateral_positions_fact cp
    JOIN investments.mvposition_analytics_fact p ON p.security_sk = cp.security_sk
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_pd_specialty_finance'
);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_specialty_finance.mvcontract_summary_fact AS
SELECT cs.contract_summary_sk, cs.enterprise_key, cs.summary_date, cs.contract_sk,
    cs.outstanding_principal_usd, cs.accrued_interest_usd, cs.paid_to_date_usd,
    cs.performance_status, cs.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvcontract_summary_fact cs
WHERE cs.contract_sk IN (SELECT DISTINCT contract_sk FROM team_pd_specialty_finance.mvcontract_details_fact);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_specialty_finance.mvportfolio_analytics_fact AS
SELECT pa.portfolio_analytics_sk, pa.analytics_date, pa.portfolio_sk, pa.business_unit_sk,
    pa.var_95, pa.var_99, pa.expected_shortfall, pa.period_return_pct, pa.ytd_return_pct,
    pa.benchmark_return_pct, pa.benchmark_code, pa.silver_loaded_at,
    current_timestamp() AS gold_loaded_at
FROM investments.mvportfolio_analytics_fact pa
JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = pa.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_pd_specialty_finance';

CREATE OR REPLACE MATERIALIZED VIEW team_pd_specialty_finance.mvsecurity_dim AS
WITH team_secs AS (
    SELECT DISTINCT p.security_sk FROM investments.mvposition_analytics_fact p
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_pd_specialty_finance'
)
SELECT s.security_sk, s.enterprise_key, s.effective_start_date, s.effective_end_date, s.is_current,
    s.security_name, s.asset_class, s.sub_asset_class, s.issue_date, s.maturity_date,
    s.coupon_rate, s.currency_code, s.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_dim s JOIN team_secs ts ON ts.security_sk = s.security_sk;

CREATE OR REPLACE MATERIALIZED VIEW team_pd_specialty_finance.mvsecurity_master_fact AS
SELECT m.security_master_sk, m.snapshot_date, m.security_sk, m.enterprise_key, m.security_name,
    m.asset_class, m.latest_close_price_usd, m.days_to_maturity, m.is_matured,
    m.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_master_fact m
WHERE m.security_sk IN (SELECT security_sk FROM team_pd_specialty_finance.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_specialty_finance.mvsecurity_price_fact AS
SELECT sp.security_price_sk, sp.enterprise_key, sp.price_date, sp.security_sk,
    sp.close_price_usd, sp.currency_code, sp.price_type,
    sp.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_price_fact sp
WHERE sp.security_sk IN (SELECT security_sk FROM team_pd_specialty_finance.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_specialty_finance.mvsecurity_rating_dim AS
SELECT sr.security_rating_sk, sr.enterprise_key, sr.effective_start_date, sr.effective_end_date,
    sr.is_current, sr.security_enterprise_key, sr.rating_agency, sr.rating_value,
    sr.rating_outlook, sr.rating_numeric_score,
    sr.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_rating_dim sr
WHERE sr.security_enterprise_key IN (SELECT enterprise_key FROM team_pd_specialty_finance.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_specialty_finance.mvtransactions_collateral_exposure_fact AS
SELECT ce.collateral_exposure_sk, ce.enterprise_key, ce.exposure_date, ce.contract_sk,
    ce.exposure_amount_usd, ce.collateral_value_usd, ce.collateral_type, ce.ltv_pct,
    ce.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvtransactions_collateral_exposure_fact ce
WHERE ce.contract_sk IN (SELECT DISTINCT contract_sk FROM team_pd_specialty_finance.mvcontract_details_fact);

CREATE OR REPLACE MATERIALIZED VIEW team_pd_specialty_finance.mvtransactions_collateral_positions_fact AS
SELECT cp.collateral_position_sk, cp.enterprise_key, cp.position_date, cp.contract_sk,
    cp.security_sk, cp.position_value_usd, cp.collateral_role,
    cp.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvtransactions_collateral_positions_fact cp
WHERE cp.contract_sk IN (SELECT DISTINCT contract_sk FROM team_pd_specialty_finance.mvcontract_details_fact);

-- ============================================================================
-- team_re_core (0.1.2 — non-PD team, mirrors team_pd_distressed shape)
-- ============================================================================
CREATE OR REPLACE MATERIALIZED VIEW team_re_core.mvposition_analytics_fact AS
WITH team_positions AS (
    SELECT p.* FROM investments.mvposition_analytics_fact p
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_re_core'
)
SELECT position_sk, enterprise_key, position_date, portfolio_sk, security_sk, business_unit_sk,
    quantity, market_value_usd, book_value_usd, cost_basis_usd, unrealized_gl_usd,
    market_value_usd / NULLIF(SUM(market_value_usd) OVER (PARTITION BY position_date), 0) AS concentration_pct,
    CAST(DENSE_RANK() OVER (PARTITION BY position_date ORDER BY market_value_usd DESC) AS INT) AS portfolio_rank,
    currency_code, silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM team_positions;

CREATE OR REPLACE MATERIALIZED VIEW team_re_core.mvcontract_details_fact AS
SELECT c.contract_details_sk, c.enterprise_key, c.detail_date, c.contract_sk, c.entity_sk,
    c.contract_type, c.principal_usd, c.coupon_rate, c.spread_over_benchmark,
    c.days_to_maturity, c.status, c.has_active_breach,
    YEAR(cd.signing_date) AS vintage_year,
    c.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvcontract_details_fact c
LEFT JOIN investments.mvcontract_dim cd ON cd.contract_sk = c.contract_sk AND cd.is_current = TRUE
WHERE c.contract_sk IN (
    SELECT DISTINCT cp.contract_sk FROM investments.mvtransactions_collateral_positions_fact cp
    JOIN investments.mvposition_analytics_fact p ON p.security_sk = cp.security_sk
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_re_core'
);

CREATE OR REPLACE MATERIALIZED VIEW team_re_core.mvcontract_summary_fact AS
SELECT cs.contract_summary_sk, cs.enterprise_key, cs.summary_date, cs.contract_sk,
    cs.outstanding_principal_usd, cs.accrued_interest_usd, cs.paid_to_date_usd,
    cs.performance_status, cs.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvcontract_summary_fact cs
WHERE cs.contract_sk IN (SELECT DISTINCT contract_sk FROM team_re_core.mvcontract_details_fact);

CREATE OR REPLACE MATERIALIZED VIEW team_re_core.mvportfolio_analytics_fact AS
SELECT pa.portfolio_analytics_sk, pa.analytics_date, pa.portfolio_sk, pa.business_unit_sk,
    pa.var_95, pa.var_99, pa.expected_shortfall, pa.period_return_pct, pa.ytd_return_pct,
    pa.benchmark_return_pct, pa.benchmark_code, pa.silver_loaded_at,
    current_timestamp() AS gold_loaded_at
FROM investments.mvportfolio_analytics_fact pa
JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = pa.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_re_core';

CREATE OR REPLACE MATERIALIZED VIEW team_re_core.mvsecurity_dim AS
WITH team_secs AS (
    SELECT DISTINCT p.security_sk FROM investments.mvposition_analytics_fact p
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_re_core'
)
SELECT s.security_sk, s.enterprise_key, s.effective_start_date, s.effective_end_date, s.is_current,
    s.security_name, s.asset_class, s.sub_asset_class, s.issue_date, s.maturity_date,
    s.coupon_rate, s.currency_code, s.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_dim s JOIN team_secs ts ON ts.security_sk = s.security_sk;

CREATE OR REPLACE MATERIALIZED VIEW team_re_core.mvsecurity_master_fact AS
SELECT m.security_master_sk, m.snapshot_date, m.security_sk, m.enterprise_key, m.security_name,
    m.asset_class, m.latest_close_price_usd, m.days_to_maturity, m.is_matured,
    m.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_master_fact m
WHERE m.security_sk IN (SELECT security_sk FROM team_re_core.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_re_core.mvsecurity_price_fact AS
SELECT sp.security_price_sk, sp.enterprise_key, sp.price_date, sp.security_sk,
    sp.close_price_usd, sp.currency_code, sp.price_type,
    sp.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_price_fact sp
WHERE sp.security_sk IN (SELECT security_sk FROM team_re_core.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_re_core.mvsecurity_rating_dim AS
SELECT sr.security_rating_sk, sr.enterprise_key, sr.effective_start_date, sr.effective_end_date,
    sr.is_current, sr.security_enterprise_key, sr.rating_agency, sr.rating_value,
    sr.rating_outlook, sr.rating_numeric_score,
    sr.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_rating_dim sr
WHERE sr.security_enterprise_key IN (SELECT enterprise_key FROM team_re_core.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_re_core.mvtransactions_collateral_exposure_fact AS
SELECT ce.collateral_exposure_sk, ce.enterprise_key, ce.exposure_date, ce.contract_sk,
    ce.exposure_amount_usd, ce.collateral_value_usd, ce.collateral_type, ce.ltv_pct,
    ce.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvtransactions_collateral_exposure_fact ce
WHERE ce.contract_sk IN (SELECT DISTINCT contract_sk FROM team_re_core.mvcontract_details_fact);

CREATE OR REPLACE MATERIALIZED VIEW team_re_core.mvtransactions_collateral_positions_fact AS
SELECT cp.collateral_position_sk, cp.enterprise_key, cp.position_date, cp.contract_sk,
    cp.security_sk, cp.position_value_usd, cp.collateral_role,
    cp.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvtransactions_collateral_positions_fact cp
WHERE cp.contract_sk IN (SELECT DISTINCT contract_sk FROM team_re_core.mvcontract_details_fact);

-- ============================================================================
-- team_re_value_add (0.1.2 — non-PD team, mirrors team_pd_distressed shape)
-- ============================================================================
CREATE OR REPLACE MATERIALIZED VIEW team_re_value_add.mvposition_analytics_fact AS
WITH team_positions AS (
    SELECT p.* FROM investments.mvposition_analytics_fact p
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_re_value_add'
)
SELECT position_sk, enterprise_key, position_date, portfolio_sk, security_sk, business_unit_sk,
    quantity, market_value_usd, book_value_usd, cost_basis_usd, unrealized_gl_usd,
    market_value_usd / NULLIF(SUM(market_value_usd) OVER (PARTITION BY position_date), 0) AS concentration_pct,
    CAST(DENSE_RANK() OVER (PARTITION BY position_date ORDER BY market_value_usd DESC) AS INT) AS portfolio_rank,
    currency_code, silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM team_positions;

CREATE OR REPLACE MATERIALIZED VIEW team_re_value_add.mvcontract_details_fact AS
SELECT c.contract_details_sk, c.enterprise_key, c.detail_date, c.contract_sk, c.entity_sk,
    c.contract_type, c.principal_usd, c.coupon_rate, c.spread_over_benchmark,
    c.days_to_maturity, c.status, c.has_active_breach,
    YEAR(cd.signing_date) AS vintage_year,
    c.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvcontract_details_fact c
LEFT JOIN investments.mvcontract_dim cd ON cd.contract_sk = c.contract_sk AND cd.is_current = TRUE
WHERE c.contract_sk IN (
    SELECT DISTINCT cp.contract_sk FROM investments.mvtransactions_collateral_positions_fact cp
    JOIN investments.mvposition_analytics_fact p ON p.security_sk = cp.security_sk
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_re_value_add'
);

CREATE OR REPLACE MATERIALIZED VIEW team_re_value_add.mvcontract_summary_fact AS
SELECT cs.contract_summary_sk, cs.enterprise_key, cs.summary_date, cs.contract_sk,
    cs.outstanding_principal_usd, cs.accrued_interest_usd, cs.paid_to_date_usd,
    cs.performance_status, cs.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvcontract_summary_fact cs
WHERE cs.contract_sk IN (SELECT DISTINCT contract_sk FROM team_re_value_add.mvcontract_details_fact);

CREATE OR REPLACE MATERIALIZED VIEW team_re_value_add.mvportfolio_analytics_fact AS
SELECT pa.portfolio_analytics_sk, pa.analytics_date, pa.portfolio_sk, pa.business_unit_sk,
    pa.var_95, pa.var_99, pa.expected_shortfall, pa.period_return_pct, pa.ytd_return_pct,
    pa.benchmark_return_pct, pa.benchmark_code, pa.silver_loaded_at,
    current_timestamp() AS gold_loaded_at
FROM investments.mvportfolio_analytics_fact pa
JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = pa.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_re_value_add';

CREATE OR REPLACE MATERIALIZED VIEW team_re_value_add.mvsecurity_dim AS
WITH team_secs AS (
    SELECT DISTINCT p.security_sk FROM investments.mvposition_analytics_fact p
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_re_value_add'
)
SELECT s.security_sk, s.enterprise_key, s.effective_start_date, s.effective_end_date, s.is_current,
    s.security_name, s.asset_class, s.sub_asset_class, s.issue_date, s.maturity_date,
    s.coupon_rate, s.currency_code, s.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_dim s JOIN team_secs ts ON ts.security_sk = s.security_sk;

CREATE OR REPLACE MATERIALIZED VIEW team_re_value_add.mvsecurity_master_fact AS
SELECT m.security_master_sk, m.snapshot_date, m.security_sk, m.enterprise_key, m.security_name,
    m.asset_class, m.latest_close_price_usd, m.days_to_maturity, m.is_matured,
    m.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_master_fact m
WHERE m.security_sk IN (SELECT security_sk FROM team_re_value_add.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_re_value_add.mvsecurity_price_fact AS
SELECT sp.security_price_sk, sp.enterprise_key, sp.price_date, sp.security_sk,
    sp.close_price_usd, sp.currency_code, sp.price_type,
    sp.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_price_fact sp
WHERE sp.security_sk IN (SELECT security_sk FROM team_re_value_add.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_re_value_add.mvsecurity_rating_dim AS
SELECT sr.security_rating_sk, sr.enterprise_key, sr.effective_start_date, sr.effective_end_date,
    sr.is_current, sr.security_enterprise_key, sr.rating_agency, sr.rating_value,
    sr.rating_outlook, sr.rating_numeric_score,
    sr.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_rating_dim sr
WHERE sr.security_enterprise_key IN (SELECT enterprise_key FROM team_re_value_add.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_re_value_add.mvtransactions_collateral_exposure_fact AS
SELECT ce.collateral_exposure_sk, ce.enterprise_key, ce.exposure_date, ce.contract_sk,
    ce.exposure_amount_usd, ce.collateral_value_usd, ce.collateral_type, ce.ltv_pct,
    ce.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvtransactions_collateral_exposure_fact ce
WHERE ce.contract_sk IN (SELECT DISTINCT contract_sk FROM team_re_value_add.mvcontract_details_fact);

CREATE OR REPLACE MATERIALIZED VIEW team_re_value_add.mvtransactions_collateral_positions_fact AS
SELECT cp.collateral_position_sk, cp.enterprise_key, cp.position_date, cp.contract_sk,
    cp.security_sk, cp.position_value_usd, cp.collateral_role,
    cp.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvtransactions_collateral_positions_fact cp
WHERE cp.contract_sk IN (SELECT DISTINCT contract_sk FROM team_re_value_add.mvcontract_details_fact);

-- ============================================================================
-- team_pe_buyout (0.1.2 — non-PD team, mirrors team_pd_distressed shape)
-- ============================================================================
CREATE OR REPLACE MATERIALIZED VIEW team_pe_buyout.mvposition_analytics_fact AS
WITH team_positions AS (
    SELECT p.* FROM investments.mvposition_analytics_fact p
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_pe_buyout'
)
SELECT position_sk, enterprise_key, position_date, portfolio_sk, security_sk, business_unit_sk,
    quantity, market_value_usd, book_value_usd, cost_basis_usd, unrealized_gl_usd,
    market_value_usd / NULLIF(SUM(market_value_usd) OVER (PARTITION BY position_date), 0) AS concentration_pct,
    CAST(DENSE_RANK() OVER (PARTITION BY position_date ORDER BY market_value_usd DESC) AS INT) AS portfolio_rank,
    currency_code, silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM team_positions;

CREATE OR REPLACE MATERIALIZED VIEW team_pe_buyout.mvcontract_details_fact AS
SELECT c.contract_details_sk, c.enterprise_key, c.detail_date, c.contract_sk, c.entity_sk,
    c.contract_type, c.principal_usd, c.coupon_rate, c.spread_over_benchmark,
    c.days_to_maturity, c.status, c.has_active_breach,
    YEAR(cd.signing_date) AS vintage_year,
    c.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvcontract_details_fact c
LEFT JOIN investments.mvcontract_dim cd ON cd.contract_sk = c.contract_sk AND cd.is_current = TRUE
WHERE c.contract_sk IN (
    SELECT DISTINCT cp.contract_sk FROM investments.mvtransactions_collateral_positions_fact cp
    JOIN investments.mvposition_analytics_fact p ON p.security_sk = cp.security_sk
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_pe_buyout'
);

CREATE OR REPLACE MATERIALIZED VIEW team_pe_buyout.mvcontract_summary_fact AS
SELECT cs.contract_summary_sk, cs.enterprise_key, cs.summary_date, cs.contract_sk,
    cs.outstanding_principal_usd, cs.accrued_interest_usd, cs.paid_to_date_usd,
    cs.performance_status, cs.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvcontract_summary_fact cs
WHERE cs.contract_sk IN (SELECT DISTINCT contract_sk FROM team_pe_buyout.mvcontract_details_fact);

CREATE OR REPLACE MATERIALIZED VIEW team_pe_buyout.mvportfolio_analytics_fact AS
SELECT pa.portfolio_analytics_sk, pa.analytics_date, pa.portfolio_sk, pa.business_unit_sk,
    pa.var_95, pa.var_99, pa.expected_shortfall, pa.period_return_pct, pa.ytd_return_pct,
    pa.benchmark_return_pct, pa.benchmark_code, pa.silver_loaded_at,
    current_timestamp() AS gold_loaded_at
FROM investments.mvportfolio_analytics_fact pa
JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = pa.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_pe_buyout';

CREATE OR REPLACE MATERIALIZED VIEW team_pe_buyout.mvsecurity_dim AS
WITH team_secs AS (
    SELECT DISTINCT p.security_sk FROM investments.mvposition_analytics_fact p
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_pe_buyout'
)
SELECT s.security_sk, s.enterprise_key, s.effective_start_date, s.effective_end_date, s.is_current,
    s.security_name, s.asset_class, s.sub_asset_class, s.issue_date, s.maturity_date,
    s.coupon_rate, s.currency_code, s.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_dim s JOIN team_secs ts ON ts.security_sk = s.security_sk;

CREATE OR REPLACE MATERIALIZED VIEW team_pe_buyout.mvsecurity_master_fact AS
SELECT m.security_master_sk, m.snapshot_date, m.security_sk, m.enterprise_key, m.security_name,
    m.asset_class, m.latest_close_price_usd, m.days_to_maturity, m.is_matured,
    m.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_master_fact m
WHERE m.security_sk IN (SELECT security_sk FROM team_pe_buyout.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_pe_buyout.mvsecurity_price_fact AS
SELECT sp.security_price_sk, sp.enterprise_key, sp.price_date, sp.security_sk,
    sp.close_price_usd, sp.currency_code, sp.price_type,
    sp.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_price_fact sp
WHERE sp.security_sk IN (SELECT security_sk FROM team_pe_buyout.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_pe_buyout.mvsecurity_rating_dim AS
SELECT sr.security_rating_sk, sr.enterprise_key, sr.effective_start_date, sr.effective_end_date,
    sr.is_current, sr.security_enterprise_key, sr.rating_agency, sr.rating_value,
    sr.rating_outlook, sr.rating_numeric_score,
    sr.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_rating_dim sr
WHERE sr.security_enterprise_key IN (SELECT enterprise_key FROM team_pe_buyout.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_pe_buyout.mvtransactions_collateral_exposure_fact AS
SELECT ce.collateral_exposure_sk, ce.enterprise_key, ce.exposure_date, ce.contract_sk,
    ce.exposure_amount_usd, ce.collateral_value_usd, ce.collateral_type, ce.ltv_pct,
    ce.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvtransactions_collateral_exposure_fact ce
WHERE ce.contract_sk IN (SELECT DISTINCT contract_sk FROM team_pe_buyout.mvcontract_details_fact);

CREATE OR REPLACE MATERIALIZED VIEW team_pe_buyout.mvtransactions_collateral_positions_fact AS
SELECT cp.collateral_position_sk, cp.enterprise_key, cp.position_date, cp.contract_sk,
    cp.security_sk, cp.position_value_usd, cp.collateral_role,
    cp.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvtransactions_collateral_positions_fact cp
WHERE cp.contract_sk IN (SELECT DISTINCT contract_sk FROM team_pe_buyout.mvcontract_details_fact);

-- ============================================================================
-- team_infra (0.1.2 — non-PD team, mirrors team_pd_distressed shape)
-- ============================================================================
CREATE OR REPLACE MATERIALIZED VIEW team_infra.mvposition_analytics_fact AS
WITH team_positions AS (
    SELECT p.* FROM investments.mvposition_analytics_fact p
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_infra'
)
SELECT position_sk, enterprise_key, position_date, portfolio_sk, security_sk, business_unit_sk,
    quantity, market_value_usd, book_value_usd, cost_basis_usd, unrealized_gl_usd,
    market_value_usd / NULLIF(SUM(market_value_usd) OVER (PARTITION BY position_date), 0) AS concentration_pct,
    CAST(DENSE_RANK() OVER (PARTITION BY position_date ORDER BY market_value_usd DESC) AS INT) AS portfolio_rank,
    currency_code, silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM team_positions;

CREATE OR REPLACE MATERIALIZED VIEW team_infra.mvcontract_details_fact AS
SELECT c.contract_details_sk, c.enterprise_key, c.detail_date, c.contract_sk, c.entity_sk,
    c.contract_type, c.principal_usd, c.coupon_rate, c.spread_over_benchmark,
    c.days_to_maturity, c.status, c.has_active_breach,
    YEAR(cd.signing_date) AS vintage_year,
    c.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvcontract_details_fact c
LEFT JOIN investments.mvcontract_dim cd ON cd.contract_sk = c.contract_sk AND cd.is_current = TRUE
WHERE c.contract_sk IN (
    SELECT DISTINCT cp.contract_sk FROM investments.mvtransactions_collateral_positions_fact cp
    JOIN investments.mvposition_analytics_fact p ON p.security_sk = cp.security_sk
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_infra'
);

CREATE OR REPLACE MATERIALIZED VIEW team_infra.mvcontract_summary_fact AS
SELECT cs.contract_summary_sk, cs.enterprise_key, cs.summary_date, cs.contract_sk,
    cs.outstanding_principal_usd, cs.accrued_interest_usd, cs.paid_to_date_usd,
    cs.performance_status, cs.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvcontract_summary_fact cs
WHERE cs.contract_sk IN (SELECT DISTINCT contract_sk FROM team_infra.mvcontract_details_fact);

CREATE OR REPLACE MATERIALIZED VIEW team_infra.mvportfolio_analytics_fact AS
SELECT pa.portfolio_analytics_sk, pa.analytics_date, pa.portfolio_sk, pa.business_unit_sk,
    pa.var_95, pa.var_99, pa.expected_shortfall, pa.period_return_pct, pa.ytd_return_pct,
    pa.benchmark_return_pct, pa.benchmark_code, pa.silver_loaded_at,
    current_timestamp() AS gold_loaded_at
FROM investments.mvportfolio_analytics_fact pa
JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = pa.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_infra';

CREATE OR REPLACE MATERIALIZED VIEW team_infra.mvsecurity_dim AS
WITH team_secs AS (
    SELECT DISTINCT p.security_sk FROM investments.mvposition_analytics_fact p
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_infra'
)
SELECT s.security_sk, s.enterprise_key, s.effective_start_date, s.effective_end_date, s.is_current,
    s.security_name, s.asset_class, s.sub_asset_class, s.issue_date, s.maturity_date,
    s.coupon_rate, s.currency_code, s.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_dim s JOIN team_secs ts ON ts.security_sk = s.security_sk;

CREATE OR REPLACE MATERIALIZED VIEW team_infra.mvsecurity_master_fact AS
SELECT m.security_master_sk, m.snapshot_date, m.security_sk, m.enterprise_key, m.security_name,
    m.asset_class, m.latest_close_price_usd, m.days_to_maturity, m.is_matured,
    m.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_master_fact m
WHERE m.security_sk IN (SELECT security_sk FROM team_infra.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_infra.mvsecurity_price_fact AS
SELECT sp.security_price_sk, sp.enterprise_key, sp.price_date, sp.security_sk,
    sp.close_price_usd, sp.currency_code, sp.price_type,
    sp.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_price_fact sp
WHERE sp.security_sk IN (SELECT security_sk FROM team_infra.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_infra.mvsecurity_rating_dim AS
SELECT sr.security_rating_sk, sr.enterprise_key, sr.effective_start_date, sr.effective_end_date,
    sr.is_current, sr.security_enterprise_key, sr.rating_agency, sr.rating_value,
    sr.rating_outlook, sr.rating_numeric_score,
    sr.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_rating_dim sr
WHERE sr.security_enterprise_key IN (SELECT enterprise_key FROM team_infra.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_infra.mvtransactions_collateral_exposure_fact AS
SELECT ce.collateral_exposure_sk, ce.enterprise_key, ce.exposure_date, ce.contract_sk,
    ce.exposure_amount_usd, ce.collateral_value_usd, ce.collateral_type, ce.ltv_pct,
    ce.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvtransactions_collateral_exposure_fact ce
WHERE ce.contract_sk IN (SELECT DISTINCT contract_sk FROM team_infra.mvcontract_details_fact);

CREATE OR REPLACE MATERIALIZED VIEW team_infra.mvtransactions_collateral_positions_fact AS
SELECT cp.collateral_position_sk, cp.enterprise_key, cp.position_date, cp.contract_sk,
    cp.security_sk, cp.position_value_usd, cp.collateral_role,
    cp.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvtransactions_collateral_positions_fact cp
WHERE cp.contract_sk IN (SELECT DISTINCT contract_sk FROM team_infra.mvcontract_details_fact);

-- ============================================================================
-- team_public_equity (0.1.2 — non-PD team, mirrors team_pd_distressed shape)
-- ============================================================================
CREATE OR REPLACE MATERIALIZED VIEW team_public_equity.mvposition_analytics_fact AS
WITH team_positions AS (
    SELECT p.* FROM investments.mvposition_analytics_fact p
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_public_equity'
)
SELECT position_sk, enterprise_key, position_date, portfolio_sk, security_sk, business_unit_sk,
    quantity, market_value_usd, book_value_usd, cost_basis_usd, unrealized_gl_usd,
    market_value_usd / NULLIF(SUM(market_value_usd) OVER (PARTITION BY position_date), 0) AS concentration_pct,
    CAST(DENSE_RANK() OVER (PARTITION BY position_date ORDER BY market_value_usd DESC) AS INT) AS portfolio_rank,
    currency_code, silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM team_positions;

CREATE OR REPLACE MATERIALIZED VIEW team_public_equity.mvcontract_details_fact AS
SELECT c.contract_details_sk, c.enterprise_key, c.detail_date, c.contract_sk, c.entity_sk,
    c.contract_type, c.principal_usd, c.coupon_rate, c.spread_over_benchmark,
    c.days_to_maturity, c.status, c.has_active_breach,
    YEAR(cd.signing_date) AS vintage_year,
    c.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvcontract_details_fact c
LEFT JOIN investments.mvcontract_dim cd ON cd.contract_sk = c.contract_sk AND cd.is_current = TRUE
WHERE c.contract_sk IN (
    SELECT DISTINCT cp.contract_sk FROM investments.mvtransactions_collateral_positions_fact cp
    JOIN investments.mvposition_analytics_fact p ON p.security_sk = cp.security_sk
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_public_equity'
);

CREATE OR REPLACE MATERIALIZED VIEW team_public_equity.mvcontract_summary_fact AS
SELECT cs.contract_summary_sk, cs.enterprise_key, cs.summary_date, cs.contract_sk,
    cs.outstanding_principal_usd, cs.accrued_interest_usd, cs.paid_to_date_usd,
    cs.performance_status, cs.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvcontract_summary_fact cs
WHERE cs.contract_sk IN (SELECT DISTINCT contract_sk FROM team_public_equity.mvcontract_details_fact);

CREATE OR REPLACE MATERIALIZED VIEW team_public_equity.mvportfolio_analytics_fact AS
SELECT pa.portfolio_analytics_sk, pa.analytics_date, pa.portfolio_sk, pa.business_unit_sk,
    pa.var_95, pa.var_99, pa.expected_shortfall, pa.period_return_pct, pa.ytd_return_pct,
    pa.benchmark_return_pct, pa.benchmark_code, pa.silver_loaded_at,
    current_timestamp() AS gold_loaded_at
FROM investments.mvportfolio_analytics_fact pa
JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = pa.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_public_equity';

CREATE OR REPLACE MATERIALIZED VIEW team_public_equity.mvsecurity_dim AS
WITH team_secs AS (
    SELECT DISTINCT p.security_sk FROM investments.mvposition_analytics_fact p
    JOIN investments.mvbusiness_unit_dim bu ON bu.business_unit_sk = p.business_unit_sk AND bu.is_current = TRUE AND bu.bu_code = 'team_public_equity'
)
SELECT s.security_sk, s.enterprise_key, s.effective_start_date, s.effective_end_date, s.is_current,
    s.security_name, s.asset_class, s.sub_asset_class, s.issue_date, s.maturity_date,
    s.coupon_rate, s.currency_code, s.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_dim s JOIN team_secs ts ON ts.security_sk = s.security_sk;

CREATE OR REPLACE MATERIALIZED VIEW team_public_equity.mvsecurity_master_fact AS
SELECT m.security_master_sk, m.snapshot_date, m.security_sk, m.enterprise_key, m.security_name,
    m.asset_class, m.latest_close_price_usd, m.days_to_maturity, m.is_matured,
    m.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_master_fact m
WHERE m.security_sk IN (SELECT security_sk FROM team_public_equity.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_public_equity.mvsecurity_price_fact AS
SELECT sp.security_price_sk, sp.enterprise_key, sp.price_date, sp.security_sk,
    sp.close_price_usd, sp.currency_code, sp.price_type,
    sp.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_price_fact sp
WHERE sp.security_sk IN (SELECT security_sk FROM team_public_equity.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_public_equity.mvsecurity_rating_dim AS
SELECT sr.security_rating_sk, sr.enterprise_key, sr.effective_start_date, sr.effective_end_date,
    sr.is_current, sr.security_enterprise_key, sr.rating_agency, sr.rating_value,
    sr.rating_outlook, sr.rating_numeric_score,
    sr.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvsecurity_rating_dim sr
WHERE sr.security_enterprise_key IN (SELECT enterprise_key FROM team_public_equity.mvsecurity_dim);

CREATE OR REPLACE MATERIALIZED VIEW team_public_equity.mvtransactions_collateral_exposure_fact AS
SELECT ce.collateral_exposure_sk, ce.enterprise_key, ce.exposure_date, ce.contract_sk,
    ce.exposure_amount_usd, ce.collateral_value_usd, ce.collateral_type, ce.ltv_pct,
    ce.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvtransactions_collateral_exposure_fact ce
WHERE ce.contract_sk IN (SELECT DISTINCT contract_sk FROM team_public_equity.mvcontract_details_fact);

CREATE OR REPLACE MATERIALIZED VIEW team_public_equity.mvtransactions_collateral_positions_fact AS
SELECT cp.collateral_position_sk, cp.enterprise_key, cp.position_date, cp.contract_sk,
    cp.security_sk, cp.position_value_usd, cp.collateral_role,
    cp.silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM investments.mvtransactions_collateral_positions_fact cp
WHERE cp.contract_sk IN (SELECT DISTINCT contract_sk FROM team_public_equity.mvcontract_details_fact);


-- ============================================================================
-- SECTION G — gold_pd_consolidated (3 cascading-MV cross-team UNIONs)
--
-- Mechanically derived from gold_pd_consolidated.v* in 03_views.sql via
-- `s/v/mv/g` at every upstream ref (DECISIONS.md #13). position_book +
-- contract_book cascade through team_pd_*.mv*; transaction_book cascades
-- through investments.mvtransaction_fact (silver) which in turn reads
-- bronze.mvtransaction (DECISIONS.md #15).
-- ============================================================================

-- G.1 mvpd_position_book -----------------------------------------------------
CREATE OR REPLACE MATERIALIZED VIEW gold_pd_consolidated.mvpd_position_book AS
WITH cross_team AS (
    SELECT 'team_pd_direct_lending' AS team_code,
           p.enterprise_key, p.position_date, p.portfolio_sk, p.security_sk, p.business_unit_sk,
           p.market_value_usd, p.book_value_usd, p.cost_basis_usd, p.unrealized_gl_usd,
           p.concentration_pct, p.portfolio_rank, p.currency_code, p.silver_loaded_at
    FROM team_pd_direct_lending.mvposition_analytics_fact p
    UNION ALL
    SELECT 'team_pd_distressed', p.enterprise_key, p.position_date, p.portfolio_sk, p.security_sk, p.business_unit_sk,
           p.market_value_usd, p.book_value_usd, p.cost_basis_usd, p.unrealized_gl_usd,
           p.concentration_pct, p.portfolio_rank, p.currency_code, p.silver_loaded_at
    FROM team_pd_distressed.mvposition_analytics_fact p
    UNION ALL
    SELECT 'team_pd_mezzanine', p.enterprise_key, p.position_date, p.portfolio_sk, p.security_sk, p.business_unit_sk,
           p.market_value_usd, p.book_value_usd, p.cost_basis_usd, p.unrealized_gl_usd,
           p.concentration_pct, p.portfolio_rank, p.currency_code, p.silver_loaded_at
    FROM team_pd_mezzanine.mvposition_analytics_fact p
    UNION ALL
    SELECT 'team_pd_real_estate_debt', p.enterprise_key, p.position_date, p.portfolio_sk, p.security_sk, p.business_unit_sk,
           p.market_value_usd, p.book_value_usd, p.cost_basis_usd, p.unrealized_gl_usd,
           p.concentration_pct, p.portfolio_rank, p.currency_code, p.silver_loaded_at
    FROM team_pd_real_estate_debt.mvposition_analytics_fact p
    UNION ALL
    SELECT 'team_pd_specialty_finance', p.enterprise_key, p.position_date, p.portfolio_sk, p.security_sk, p.business_unit_sk,
           p.market_value_usd, p.book_value_usd, p.cost_basis_usd, p.unrealized_gl_usd,
           p.concentration_pct, p.portfolio_rank, p.currency_code, p.silver_loaded_at
    FROM team_pd_specialty_finance.mvposition_analytics_fact p
)
SELECT
    CAST(ROW_NUMBER() OVER (ORDER BY position_date, team_code, enterprise_key) AS BIGINT) AS pd_position_book_sk,
    team_code, enterprise_key, position_date,
    portfolio_sk, security_sk, business_unit_sk,
    market_value_usd, book_value_usd, cost_basis_usd, unrealized_gl_usd,
    concentration_pct, portfolio_rank, currency_code,
    silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM cross_team;

-- G.2 mvpd_contract_book -----------------------------------------------------
CREATE OR REPLACE MATERIALIZED VIEW gold_pd_consolidated.mvpd_contract_book AS
WITH cross_team AS (
    SELECT 'team_pd_direct_lending' AS team_code,
           cs.enterprise_key, cs.summary_date, cs.contract_sk,
           cs.outstanding_principal_usd, cs.accrued_interest_usd, cs.paid_to_date_usd,
           cs.performance_status,
           COALESCE(cd.has_active_breach, FALSE) AS has_active_breach,
           cs.silver_loaded_at
    FROM team_pd_direct_lending.mvcontract_summary_fact cs
    LEFT JOIN team_pd_direct_lending.mvcontract_details_fact cd
        ON cd.contract_sk = cs.contract_sk AND cd.detail_date = cs.summary_date
    UNION ALL
    SELECT 'team_pd_distressed', cs.enterprise_key, cs.summary_date, cs.contract_sk,
           cs.outstanding_principal_usd, cs.accrued_interest_usd, cs.paid_to_date_usd, cs.performance_status,
           COALESCE(cd.has_active_breach, FALSE), cs.silver_loaded_at
    FROM team_pd_distressed.mvcontract_summary_fact cs
    LEFT JOIN team_pd_distressed.mvcontract_details_fact cd
        ON cd.contract_sk = cs.contract_sk AND cd.detail_date = cs.summary_date
    UNION ALL
    SELECT 'team_pd_mezzanine', cs.enterprise_key, cs.summary_date, cs.contract_sk,
           cs.outstanding_principal_usd, cs.accrued_interest_usd, cs.paid_to_date_usd, cs.performance_status,
           COALESCE(cd.has_active_breach, FALSE), cs.silver_loaded_at
    FROM team_pd_mezzanine.mvcontract_summary_fact cs
    LEFT JOIN team_pd_mezzanine.mvcontract_details_fact cd
        ON cd.contract_sk = cs.contract_sk AND cd.detail_date = cs.summary_date
    UNION ALL
    SELECT 'team_pd_real_estate_debt', cs.enterprise_key, cs.summary_date, cs.contract_sk,
           cs.outstanding_principal_usd, cs.accrued_interest_usd, cs.paid_to_date_usd, cs.performance_status,
           COALESCE(cd.has_active_breach, FALSE), cs.silver_loaded_at
    FROM team_pd_real_estate_debt.mvcontract_summary_fact cs
    LEFT JOIN team_pd_real_estate_debt.mvcontract_details_fact cd
        ON cd.contract_sk = cs.contract_sk AND cd.detail_date = cs.summary_date
    UNION ALL
    SELECT 'team_pd_specialty_finance', cs.enterprise_key, cs.summary_date, cs.contract_sk,
           cs.outstanding_principal_usd, cs.accrued_interest_usd, cs.paid_to_date_usd, cs.performance_status,
           COALESCE(cd.has_active_breach, FALSE), cs.silver_loaded_at
    FROM team_pd_specialty_finance.mvcontract_summary_fact cs
    LEFT JOIN team_pd_specialty_finance.mvcontract_details_fact cd
        ON cd.contract_sk = cs.contract_sk AND cd.detail_date = cs.summary_date
)
SELECT
    CAST(ROW_NUMBER() OVER (ORDER BY summary_date, team_code, enterprise_key) AS BIGINT) AS pd_contract_book_sk,
    team_code, enterprise_key, summary_date, contract_sk,
    outstanding_principal_usd, accrued_interest_usd, paid_to_date_usd,
    performance_status, has_active_breach,
    silver_loaded_at, current_timestamp() AS gold_loaded_at
FROM cross_team;

-- G.3 mvpd_transaction_book --------------------------------------------------
CREATE OR REPLACE MATERIALIZED VIEW gold_pd_consolidated.mvpd_transaction_book AS
WITH pd_portfolios AS (
    SELECT p.portfolio_sk,
           bu.bu_code AS team_code
    FROM investments.mvportfolio_dim p
    JOIN investments.mvbusiness_unit_dim bu
        ON bu.enterprise_key = p.business_unit_enterprise_key
       AND bu.is_current = TRUE
    WHERE p.is_current = TRUE
      AND bu.is_pd_strategy = TRUE
)
SELECT
    CAST(ROW_NUMBER() OVER (ORDER BY t.transaction_date, pp.team_code, t.enterprise_key) AS BIGINT) AS pd_transaction_book_sk,
    pp.team_code,
    t.enterprise_key, t.transaction_date, t.settlement_date,
    t.portfolio_sk, t.security_sk,
    t.transaction_type,
    t.gross_amount_usd,
    t.net_amount_usd,
    t.fees_usd,
    t.counterparty_name, t.currency_code,
    t.silver_loaded_at,
    current_timestamp() AS gold_loaded_at
FROM investments.mvtransaction_fact t
JOIN pd_portfolios pp ON pp.portfolio_sk = t.portfolio_sk;

SELECT 'gold.materialized_views complete' AS status,
       count(*) AS gold_mv_count
FROM information_schema.tables
WHERE table_schema IN ('team_pd_direct_lending','team_pd_distressed','team_pd_mezzanine',
                       'team_pd_real_estate_debt','team_pd_specialty_finance','gold_pd_consolidated',
                       'team_re_core','team_re_value_add','team_pe_buyout','team_infra','team_public_equity')
  AND table_name LIKE 'mv%';
