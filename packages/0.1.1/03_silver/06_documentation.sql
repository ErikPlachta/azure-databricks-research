...
ALTER TABLE investments_history.t_vposition_monthend_fact ALTER COLUMN monthend_date    COMMENT 'last_day(position_date). Pre-aggregated period-end snapshot from vposition_analytics_fact.';
...
ALTER TABLE investments_history.t_vcontract_details_cancels_fact ALTER COLUMN cancel_reason COMMENT 'AMENDMENT (in 0.1.0 — derived from _v2 contract source_keys per Phase 6.4 of seed). Future: RESTATEMENT / VOID / DUPLICATE.';
ALTER TABLE investments_history.t_vposition_cancels_fact ALTER COLUMN cancel_reason         COMMENT 'CROSSWALK_REMAP (in 0.1.0 — derived from REMAPPED_ enterprise_keys per Phase 6.5 of seed). Future: CUSTODIAN_RESTATEMENT.';
ALTER TABLE investments_history.t_vsecurity_price_cancels_fact ALTER COLUMN cancel_reason   COMMENT 'Reserved (0.1.0 emits 0 rows). Phase 6 doesn''t model price cancels yet.';

-- ============================================================================
-- BRIDGE
-- ============================================================================

ALTER TABLE investments_history.t_vincome_bridge ALTER COLUMN income_type COMMENT 'COUPON / DIVIDEND / FEE_INCOME / OTHER. 0.1.0 simplification: filters cash_flow_raw.cash_flow_type = INCOME, all bucketed as COUPON. Future: distinguish dividend vs coupon vs fee income.';
ALTER TABLE investments_history.t_vincome_bridge ALTER COLUMN security_sk COMMENT 'NULL in 0.1.0: cash_flow_raw lacks security_source_key. Future: link via (portfolio + date + amount) heuristic.';
