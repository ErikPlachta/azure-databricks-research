...
CREATE OR REPLACE PROCEDURE investments_history.refresh_position_monthend_fact()
LANGUAGE SQL
SQL SECURITY INVOKER
AS BEGIN
    INSERT OVERWRITE investments_history.t_vposition_monthend_fact SELECT * FROM investments_history.vposition_monthend_fact;
END;

CREATE OR REPLACE PROCEDURE investments_history.refresh_portfolio_analytics_monthend_fact()
LANGUAGE SQL
SQL SECURITY INVOKER
AS BEGIN
    INSERT OVERWRITE investments_history.t_vportfolio_analytics_monthend_fact SELECT * FROM investments_history.vportfolio_analytics_monthend_fact;
END;
...
CREATE OR REPLACE PROCEDURE investments_history.refresh_contract_details_cancels_fact()
LANGUAGE SQL
SQL SECURITY INVOKER
AS BEGIN
    INSERT OVERWRITE investments_history.t_vcontract_details_cancels_fact SELECT * FROM investments_history.vcontract_details_cancels_fact;
END;

CREATE OR REPLACE PROCEDURE investments_history.refresh_position_cancels_fact()
LANGUAGE SQL
SQL SECURITY INVOKER
AS BEGIN
    INSERT OVERWRITE investments_history.t_vposition_cancels_fact SELECT * FROM investments_history.vposition_cancels_fact;
END;

CREATE OR REPLACE PROCEDURE investments_history.refresh_security_price_cancels_fact()
LANGUAGE SQL
SQL SECURITY INVOKER
AS BEGIN
    INSERT OVERWRITE investments_history.t_vsecurity_price_cancels_fact SELECT * FROM investments_history.vsecurity_price_cancels_fact;
END;
...
CREATE OR REPLACE PROCEDURE investments_history.refresh_income_bridge()
LANGUAGE SQL
SQL SECURITY INVOKER
AS BEGIN
    INSERT OVERWRITE investments_history.t_vincome_bridge SELECT * FROM investments_history.vincome_bridge;
END;
...
AS BEGIN
    ...
    -- Cancels
    CALL investments_history.refresh_contract_details_cancels_fact();
    CALL investments_history.refresh_position_cancels_fact();
    CALL investments_history.refresh_security_price_cancels_fact();
    -- Bridge
    CALL investments_history.refresh_income_bridge();
END;
