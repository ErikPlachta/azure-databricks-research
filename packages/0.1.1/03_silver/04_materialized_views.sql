...
-- 23 silver mv<entity> materialized views (17 investments + 6 investments_history).
--
-- Cascading-MV bodies (DECISIONS.md #13 — supersedes #6 for 0.1.1+):
--   * mv* bodies reference bronze.mv* (NOT bronze.v*) so silver MV refresh
--     reads already-materialized upstream rather than re-cascading through
--     bronze views every time.
--   * v* / mv* projections are no longer byte-identical, but ARE mechanically
--     derivable via s/v/mv/g substitution at every upstream FROM/JOIN/IN ref.
--   * v* path stays slow (production-faithful through view stack);
--     mv* path is fast (cascading materialized).
-- ============================================================================
...
CREATE OR REPLACE MATERIALIZED VIEW investments_history.mvposition_monthend_fact AS
WITH monthends AS (
...
CREATE OR REPLACE MATERIALIZED VIEW investments_history.mvportfolio_analytics_monthend_fact AS
WITH aggregated AS (
    SELECT
...
CREATE OR REPLACE MATERIALIZED VIEW investments_history.mvposition_cancels_fact AS
WITH p_resolved AS (
    SELECT p.*,
...
CREATE OR REPLACE MATERIALIZED VIEW investments_history.mvsecurity_price_cancels_fact AS
SELECT
...
CREATE OR REPLACE MATERIALIZED VIEW investments_history.mvincome_bridge AS
WITH cf_resolved AS (
