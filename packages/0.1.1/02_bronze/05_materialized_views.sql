-- ============================================================================
-- 02_bronze/05_materialized_views.sql
-- 14 bronze mv<entity> materialized views.
--
-- Bodies are byte-identical to bronze.v<entity> in 04_views.sql (modulo
-- `CREATE OR REPLACE VIEW <name>` -> `CREATE OR REPLACE MATERIALIZED VIEW
-- <name>`). This is the load-bearing apples-to-apples invariant for the
-- timing comparisons in 06_demos/. See DECISIONS.md #6.
--
-- Refresh: manual via REFRESH MATERIALIZED VIEW bronze.mv<entity>; the
-- 03_refresh_orchestrator.sql proc does NOT refresh MVs (it refreshes
-- t_<entity> tables via INSERT OVERWRITE). MV refresh is Databricks-managed.
--
-- SCHEDULE clauses are intentionally omitted — production scheduling is
-- deferred to 0.1.4 (see PLAN.md).
-- ============================================================================
...
-- 5.1 mvsecurity --------------------------------------------------------------
CREATE OR REPLACE MATERIALIZED VIEW bronze.mvsecurity AS
...
-- 5.2 mventity ----------------------------------------------------------------
CREATE OR REPLACE MATERIALIZED VIEW bronze.mventity AS
WITH aspen_latest AS (
    SELECT *
    FROM (
        SELECT
...
-- 5.3 mvasset -----------------------------------------------------------------
CREATE OR REPLACE MATERIALIZED VIEW bronze.mvasset AS
SELECT
    a.enterprise_key,
    a.asset_name,
    a.asset_type,
...
-- 5.4 mvportfolio -------------------------------------------------------------
CREATE OR REPLACE MATERIALIZED VIEW bronze.mvportfolio AS
...
-- 5.5 mvcontract --------------------------------------------------------------
CREATE OR REPLACE MATERIALIZED VIEW bronze.mvcontract AS
SELECT
    c.enterprise_key,
    c.contract_name,
...
-- 5.6 mvbusiness_unit ---------------------------------------------------------
CREATE OR REPLACE MATERIALIZED VIEW bronze.mvbusiness_unit AS
WITH bu_latest AS (
    SELECT *
    FROM (
        SELECT
...
-- 5.7 mvposition --------------------------------------------------------------
CREATE OR REPLACE MATERIALIZED VIEW bronze.mvposition AS
SELECT
    p.enterprise_key,
    p.position_date,
...
-- 5.8 mvtransaction -----------------------------------------------------------
CREATE OR REPLACE MATERIALIZED VIEW bronze.mvtransaction AS
WITH ss_settled AS (
    SELECT *
    FROM (
        SELECT
...
SELECT
    ...
    s.counterparty_name,
    s.custodian_account,
    CAST(NULL AS STRING)                    AS trade_status,
    'state_street'                          AS _source_pref,
    CAST(array() AS ARRAY<STRING>)          AS _sources_in_conflict,
...
-- 5.9 mvsecurity_price --------------------------------------------------------
CREATE OR REPLACE MATERIALIZED VIEW bronze.mvsecurity_price AS
SELECT
    sp.enterprise_key,
...
-- 5.10 mvportfolio_risk -------------------------------------------------------
CREATE OR REPLACE MATERIALIZED VIEW bronze.mvportfolio_risk AS
SELECT
    r.enterprise_key,
...
-- 5.11 mvportfolio_performance ------------------------------------------------
CREATE OR REPLACE MATERIALIZED VIEW bronze.mvportfolio_performance AS
SELECT
    p.enterprise_key,
    bronze.fn_resolve_enterprise_key('state_street', p.portfolio_source_key) AS portfolio_enterprise_key,
    p.performance_date,
...
-- 5.12 mvrating ---------------------------------------------------------------
CREATE OR REPLACE MATERIALIZED VIEW bronze.mvrating AS
SELECT
    sr.enterprise_key,
...
-- 5.13 mvcollateral -----------------------------------------------------------
CREATE OR REPLACE MATERIALIZED VIEW bronze.mvcollateral AS
SELECT
    cx.enterprise_key,
...
-- 5.14 mvfx_rate --------------------------------------------------------------
CREATE OR REPLACE MATERIALIZED VIEW bronze.mvfx_rate AS
SELECT
    fx.enterprise_key,
    fx.from_currency,
    fx.to_currency,
...
SELECT 'bronze.materialized_views complete' AS status,
       count(*) AS bronze_mv_count
